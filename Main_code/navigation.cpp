...
int turn_count = 0;     // Counter for consecutive turns
int last_turn = 0;      // 0 = no turn, 1 = left, 2 = right
const int MAX_TURNS = 5; // Max allowed consecutive left-right turns before declaring "stuck"
...
void check_if_stuck(int current_turn) {
  if (current_turn != last_turn) {
    turn_count++; // Increment if switching directions
  } else {
    turn_count = 0; // Reset if not switching directions
  }

  last_turn = current_turn; // Update last turn

  if (turn_count >= MAX_TURNS) {
    // Handle the robot being stuck
    stop();
    delay(3000);       // Stop the robot
    reverse(); 
    delay(700);
    turn_right();
    delay(300);   // Try to reverse and get out of the stuck state
    // Add any other recovery logic here (e.g., rotate, retry, etc.)
    turn_count = 0; // Reset turn counter after handling
  }
}
...
void turn_left() {
    ...
    check_if_stuck(1); // 1 represents left turn
}
...
void turn_right() {
    ...
    check_if_stuck(2); // 2 represents right turn 
}
...
void go_straight_slow() {
  navigation->Rservo.writeMicroseconds(1750);  
  navigation->Lservo.writeMicroseconds(1250); 
  // turn_count = 0;
  // last_turn = 0; 
}
